...
description: >-
  ...
  jamais dans le dépôt. Charge d'abord avqn-skill-authoring. NE COUVRE PAS les conventions
  elles-mêmes (avqn-skill-authoring), l'écriture ou la correction du skill (creer-un-skill), ni
  la relecture d'une ressource Autonomes.
couche: recette
moment: >-
...
Commencer par charger **`avqn-skill-authoring`** — le socle dont ce skill est le crible. Un
rapport ne cite jamais une règle qui n'est pas dans ce socle, ou dans le `CLAUDE.md` du dépôt
`skills`.
...
un autre geste (`creer-un-skill`, §7), sur décision de Manu.
