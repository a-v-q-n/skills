...
| `.claude/settings.json` → `extraKnownMarketplaces` + `enabledPlugins` | le plugin s'installe au démarrage **si la marketplace est lisible anonymement** (`a-v-q-n/skills` est public pour ça : le proxy GitHub n'authentifie que le repo attaché) |
| `.claude/settings.json` → hooks `SessionStart` | tournent (le hook fait `npm install` quand `CLAUDE_CODE_REMOTE=true`) |
...
- **Environnement cloud « AVQN »** (sélecteur d'environnement) :
  ...
  - setup script :
    ...
    Le cache d'environnement garde ce que le script installe ; les conteneurs se relancent par
    session (`docker run -d -p 5432:5432 -e POSTGRES_PASSWORD=dev postgres:16`). Chromium pour les
    captures : `npx playwright install --with-deps chromium` dans la session qui en a besoin (long ;
    préférer le script de captures du repo s'il existe).
...
- Pas de coffre, pas de ssh vers la prod, pas de tunnel : Postgres éphémère + migrations + fixtures.
