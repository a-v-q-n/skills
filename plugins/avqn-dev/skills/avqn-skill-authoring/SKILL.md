...
description: >-
  ...
  NE COUVRE PAS l'écriture du contenu métier d'un skill donné, ni sa relecture (relire-un-skill).
couche: socle
moment: >-
...
Un skill du métier vit dans `plugins/avqn-skills/skills/<nom>/`, un skill de méthode dans
`plugins/avqn-dev/skills/<nom>/`. Le `<nom>` est en kebab-case et devient le `name` du frontmatter
(identiques). Depuis une session attachée au dépôt, scaffolder avec `/new-skill <nom>`.
...
| `famille` | `avqn-skills` | `cycle-client`, `contenu-autonomes` ou `video-contentos` — la section de la table. Les skills d'`avqn-dev` n'en portent pas. |
...
1. La table du `README.md` **se régénère** : `python3 scripts/skills.py readme`, entre les
   marqueurs `<!-- skills:début -->` et `<!-- skills:fin -->`. On ne l'édite jamais à la main —
   pour changer une ligne, changer le `moment` ou la `couche` du skill. Cette étape vient en
   premier : la gate contrôle la fraîcheur de la table, donc l'oublier la fait rougir à coup sûr.
2. `/check-skills` — la gate du dépôt : JSON de la marketplace, frontmatter des skills et des
   agents, absence de champ `version`, présence de `couche`/`moment`/`famille`, fraîcheur de la
   table, et les tests de l'outil d'auteur.
...
Écrire depuis une conversation sans session de dev passe par le secteur `skill_` d'AVQN OPS, qui
exécute la même validation avant de commiter — la conformité y est une invariante du serveur, pas
une commande qu'on peut oublier. Tant que ce secteur n'est pas joignable, l'écriture passe par une
session attachée au dépôt, et c'est `/check-skills` qui tient le rôle.

Avant de considérer un skill fini, le passer à **`relire-un-skill`** : la conformité est binaire,
